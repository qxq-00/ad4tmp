...
from ldm.device_utils import get_device_string
...
class FrozenCLIPTextEmbedder(nn.Module):
    ...
    def __init__(self, version='ViT-L/14', device=None, max_length=77, n_repeat=1, normalize=True):
        super().__init__()
        self.device = device or default_torch_device()
        self.model, _ = clip.load(version, jit=False, device=self.device)
        self.max_length = max_length
...
class FrozenClipImageEmbedder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.device = device or default_torch_device()
        self.model, _ = clip.load(name=model, device=self.device, jit=jit)
