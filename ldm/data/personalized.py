...
class Personalized_mvtec_encoder(Dataset):
    def __init__(self,
        ...
        filter_sample_name = sample_name
        filter_anomaly_name = anomaly_name
        ...
        with open(os.path.join('name-anomaly.txt'),'r') as f:
            ...
            for i in data:
                if not i:
                    continue
                pair_sample_name,pair_anomaly_name=i.split('+')
                sample_anomaly_pairs.append((pair_sample_name,pair_anomaly_name))
        if filter_sample_name:
            sample_anomaly_pairs = [pair for pair in sample_anomaly_pairs if pair[0] == filter_sample_name]
        if filter_anomaly_name:
            sample_anomaly_pairs = [pair for pair in sample_anomaly_pairs if pair[1] == filter_anomaly_name]
        if not sample_anomaly_pairs:
            raise ValueError(
                f"No MVTec sample/anomaly pairs found for sample_name='{filter_sample_name}' "
                f"and anomaly_name='{filter_anomaly_name}'."
            )
        self.data=[]
        if data_enhance:
            size=512
        self.size = size
        self.interpolation = _resampling_filters()[interpolation]
